...
void settings_apply(const Settings *s) {
    ...
    gBloomOn         = s->bloomOn;
    gBloomIntensity  = s->bloomIntensity;
    gBloomThreshold  = s->bloomThreshold;
    gBloomRadius     = s->bloomRadius;
    gBloomQuality    = s->bloomQuality;
    gBloomTintHue    = s->bloomTintHue;
    gBloomScreen     = s->bloomScreen;
    gBloomInMenu     = s->bloomInMenu;
    gShadersOn       = s->shadersOn;
    gStreakOn        = s->streakOn;
    gStreakIntensity = s->streakIntensity;
    gStreakLength    = s->streakLength;
    gStreakTintHue   = s->streakTintHue;
    gCaOn            = s->caOn;
    gCaStrength      = s->caStrength;
    gShockOn         = s->shockOn;
    gShockStrength   = s->shockStrength;
    gShockSpeed      = s->shockSpeed;
    gBeatPunchOn       = s->beatPunchOn;
    gBeatPunchStrength = s->beatPunchStrength;
    gBeatPunchSens     = s->beatPunchSens;
...
void settings_defaults(Settings *s) {
    ...
    s->shadersOn       = false;
    s->streakOn        = false;
    s->streakIntensity = 1.0f;
    s->streakLength    = 1.5f;
    s->streakTintHue   = -1;
    s->caOn            = false;
    s->caStrength      = 1.5f;
    s->shockOn         = false;
    s->shockStrength   = 24.0f;
    s->shockSpeed      = 1.0f;
    s->beatPunchOn       = false;
    s->beatPunchStrength = 1.0f;
    s->beatPunchSens     = 1.0f;
...
void settings_load(Settings *s) {
    ...
    for (char *line = strtok(txt, "\r\n"); line; line = strtok(NULL, "\r\n")) {
        ...
        *eq = '\0'; const char *key = line, *val = eq + 1;
        ...
        else if (strcmp(key, "bloom")           == 0) s->bloomOn = (atoi(val) != 0);
        else if (strcmp(key, "bloom_intensity") == 0) s->bloomIntensity = clampf((float)atof(val), 0.0f, 3.0f);
        else if (strcmp(key, "bloom_threshold") == 0) s->bloomThreshold = clampf((float)atof(val), 0.0f, 1.0f);
        else if (strcmp(key, "bloom_radius")    == 0) s->bloomRadius = clampf((float)atof(val), 0.5f, 3.0f);
        else if (strcmp(key, "bloom_quality")   == 0) { int q = atoi(val); s->bloomQuality = q < 0 ? 0 : (q > 2 ? 2 : q); }
        else if (strcmp(key, "bloom_tint")      == 0) { int t = atoi(val); s->bloomTintHue = t < 0 ? -1 : (t % 360); }
        else if (strcmp(key, "bloom_screen")    == 0) s->bloomScreen = (atoi(val) != 0);
        else if (strcmp(key, "bloom_menu")      == 0) s->bloomInMenu = (atoi(val) != 0);
        else if (strcmp(key, "shaders")          == 0) s->shadersOn = (atoi(val) != 0);
        else if (strcmp(key, "streak")           == 0) s->streakOn = (atoi(val) != 0);
        else if (strcmp(key, "streak_intensity") == 0) s->streakIntensity = clampf((float)atof(val), 0.0f, 3.0f);
        else if (strcmp(key, "streak_length")    == 0) s->streakLength = clampf((float)atof(val), 0.5f, 4.0f);
        else if (strcmp(key, "streak_tint")      == 0) { int t = atoi(val); s->streakTintHue = t < 0 ? -1 : (t % 360); }
        else if (strcmp(key, "ca")               == 0) s->caOn = (atoi(val) != 0);
        else if (strcmp(key, "ca_strength")      == 0) s->caStrength = clampf((float)atof(val), 0.0f, 4.0f);
        else if (strcmp(key, "shock")            == 0) s->shockOn = (atoi(val) != 0);
        else if (strcmp(key, "shock_strength")   == 0) s->shockStrength = clampf((float)atof(val), 0.0f, 80.0f);
        else if (strcmp(key, "shock_speed")      == 0) s->shockSpeed = clampf((float)atof(val), 0.5f, 2.5f);
        else if (strcmp(key, "beatpunch")          == 0) s->beatPunchOn = (atoi(val) != 0);
        else if (strcmp(key, "beatpunch_strength") == 0) s->beatPunchStrength = clampf((float)atof(val), 0.0f, 2.0f);
        else if (strcmp(key, "beatpunch_sens")     == 0) s->beatPunchSens = clampf((float)atof(val), 0.5f, 2.5f);
...
void settings_save(const Settings *s) {
    ...
    o += snprintf(buf + o, sizeof buf - o, "bloom=%d\n",           s->bloomOn ? 1 : 0);
    o += snprintf(buf + o, sizeof buf - o, "bloom_intensity=%.2f\n", s->bloomIntensity);
    o += snprintf(buf + o, sizeof buf - o, "bloom_threshold=%.2f\n", s->bloomThreshold);
    o += snprintf(buf + o, sizeof buf - o, "bloom_radius=%.2f\n",  s->bloomRadius);
    o += snprintf(buf + o, sizeof buf - o, "bloom_quality=%d\n",   s->bloomQuality);
    o += snprintf(buf + o, sizeof buf - o, "bloom_tint=%d\n",      s->bloomTintHue);
    o += snprintf(buf + o, sizeof buf - o, "bloom_screen=%d\n",    s->bloomScreen ? 1 : 0);
    o += snprintf(buf + o, sizeof buf - o, "bloom_menu=%d\n",      s->bloomInMenu ? 1 : 0);
    o += snprintf(buf + o, sizeof buf - o, "shaders=%d\n",         s->shadersOn ? 1 : 0);
    o += snprintf(buf + o, sizeof buf - o, "streak=%d\n",          s->streakOn ? 1 : 0);
    o += snprintf(buf + o, sizeof buf - o, "streak_intensity=%.2f\n", s->streakIntensity);
    o += snprintf(buf + o, sizeof buf - o, "streak_length=%.2f\n", s->streakLength);
    o += snprintf(buf + o, sizeof buf - o, "streak_tint=%d\n",     s->streakTintHue);
    o += snprintf(buf + o, sizeof buf - o, "ca=%d\n",             s->caOn ? 1 : 0);
    o += snprintf(buf + o, sizeof buf - o, "ca_strength=%.2f\n",  s->caStrength);
    o += snprintf(buf + o, sizeof buf - o, "shock=%d\n",          s->shockOn ? 1 : 0);
    o += snprintf(buf + o, sizeof buf - o, "shock_strength=%.2f\n", s->shockStrength);
    o += snprintf(buf + o, sizeof buf - o, "shock_speed=%.2f\n",  s->shockSpeed);
    o += snprintf(buf + o, sizeof buf - o, "beatpunch=%d\n",          s->beatPunchOn ? 1 : 0);
    o += snprintf(buf + o, sizeof buf - o, "beatpunch_strength=%.2f\n", s->beatPunchStrength);
    o += snprintf(buf + o, sizeof buf - o, "beatpunch_sens=%.2f\n",   s->beatPunchSens);
