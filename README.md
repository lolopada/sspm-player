...
| **Drag & drop** | Drop a `.sspm` onto the window to play it instantly |
...
| **Tablet / stylus** | Absolute pointing mode |
...
| **Audio calibration** | Built-in offset-correction tool |
...
| Key / Action | Effect |
|---|---|
| `Up` / `Down` or scroll | Navigate the map list |
...
Use `Up` / `Down` to select a setting, `Left` / `Right` to change its value. Changes are saved automatically to `settings.cfg` on exit.
...
| Setting | Description |
|---|---|
| **Approach distance** | How far away notes spawn (20 to 120) |
| **Approach time** | Time for a note to reach the hit plane, in ms (200 to 1500). Lower = faster |
...
Up to **8192 maps** are loaded. Press **F5** in the menu to rescan after adding files.
...
> **`.obj` files must be fully triangulated** (no n-gons). In Blender, enable *"Triangulated Mesh"* at export, or just use `.glb`, which is always triangulated.
...
<div align="center">
  <br>
  <sub>Built with raylib · <a href="#sspm-player">back to top</a></sub>
</div>
